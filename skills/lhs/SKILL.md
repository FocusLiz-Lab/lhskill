...
name: lhs
description: |
  Leila Hormozi 领导力与商业执行 Skill 工具箱主入口。用于 CEO 领导力、招聘、团队标准、运营、决策、创始人心态、个人成长、商业执行、内容选题、资料检索、学习路径和 Leila 风格诊断。普通问题默认使用本地专家原子库和 Skill 知识包；只有用户主动调用 $lhs-ima 或明确要求 IMA 检索/引用/排错时，才检索 IMA 知识库「LeilaHormozi 知识库 | 商业实战」。触发词包括 $lhs、/lhs、Leila Hormozi、领导力、管理、招聘、团队、CEO、运营、决策、创业心态、学习地图和 IMA 检索。
---
...
普通 Leila Hormozi workflow 默认不检索 IMA。默认使用本地专家原子库和 `知识库/Skill知识包/` 方法说明作为依据。

只有当用户主动调用 `$lhs-ima`，或明确要求“检索 IMA / 引用 IMA / 排查 IMA / 从 IMA 找资料”时，才使用下面的 IMA 知识库：
...
只有 `$lhs-ima` 或用户明确要求 IMA 检索/引用/排错时，才使用 `ima-skill`。不要臆造 IMA API。不要向用户暴露内部 `knowledge_base_id`、`media_id` 或 `folder_id`。
...
- 涉及实质性观点时，默认使用本地专家原子库或 Skill 知识包依据；只有显式 IMA 请求才用 IMA。
- 区分已检索证据和框架推断。
...
1. 默认优先使用本地 Leila Hormozi 专家原子库和 `知识库/Skill知识包/` 方法说明。
2. 只有用户主动调用 `$lhs-ima`，或明确要求 IMA 检索/引用/排错时，才使用 IMA 知识库 `LeilaHormozi 知识库 | 商业实战`。
...
回答前要确保本地 Leila Hormozi 专家原子库存在。GitHub 安装的完整包通常会包含它；同时检查 `~/.agents/skills/lhs/知识库/原子库/atoms.jsonl` 和 `~/.agents/skills/lhs-download-atoms/知识库/原子库/atoms.jsonl`。如果两者都缺失，把它视为轻量/不完整安装，并调用 `$lhs-download-atoms`，或在已安装的 `lhs-download-atoms` skill 目录运行 `python tools/download_full_atoms.py`。不要询问用户是否下载；普通 workflow 默认走本地原子库，不默认检索 IMA。
